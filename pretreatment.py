...
# D:\Workspace\python\Capstone_Project\GaitSet\video-test
# D:/Workspace/python/Capstone_Project/GaitSet/output_pretreatment
...
# python pretreatment.py --input_path='D:/Workspace/python/Capstone_Project/GaitSet/CASIA-B' --output_path='D:\Workspace\python\Capstone_Project\GaitSet\CASIA-B-SMALL'
T_H = 64
T_W = 64
...
def log_print(pid, comment, logs):
    ...
    if comment in [START, FINISH]:
        if pid % 500 != 0:
            return
    print(str_log, end='')
# @cut_img: resize img follow calculated ratio ()
# @img: a not resized silhouette image
# @seq_info: path of silhouette image folder
# @frame_name: name of each silhouette image
# @pid: id
...
def cut_img(img, seq_info, frame_name, pid):
    ...
    y = img.sum(axis=1)
    y_top = (y != 0).argmax(axis=0)  # find non-zero first of column
    y_btm = (y != 0).cumsum(axis=0).argmax(
        axis=0)  # find non-zero last of column
    img = img[y_top:y_btm + 1, :]
    # As the height of a person is larger than the width,
    # use the height to calculate resize ratio.
    _r = img.shape[1] / img.shape[0]  # width / height
    _t_w = int(T_H * _r)
...
# @cut_pickle: save cut silhouette into folder
# @seq_info: path of silhouette image folder
# @pid: id
...
def cut_pickle(seq_info, pid):
    ...
    frame_list.sort()
    count_frame = 0
    for _frame_name in frame_list:  # walk through each silhouette
        frame_path = os.path.join(seq_path, _frame_name)
        img = cv2.imread(frame_path)[:, :, 0]  # convert to grayscale image
        img = cut_img(img, seq_info, _frame_name, pid)
        if img is not None:
            # Save the cut img
            print('Saved silhouette')
